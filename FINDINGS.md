...
**Snapshot:** `fda_markets_processed.csv`, data date 2026-06-24. 36 resolved
drug-specific markets (thematic psychedelic market excluded), 7 open. (Tebipenem
and Welireg both resolved Yes on approval since the prior snapshot — Welireg
2026-06-12, Tebipenem 2026-06-17 — and are now inside the 36 settled cohort.)
...
pre-identifiable refile rate is **2/5 = 40%, Wilson 95% [12%, 77%], n = 5.**
...
for** a refile penalty without **establishing its absence**: 2/5 vs 21/31 is two
under-powered cohorts whose CIs overlap ([12%,77%] vs [50%,81%]). The correct
...
|---|---|---|---|
| S1 | **The market is reasonably well-calibrated 1 day out.** | SUGGESTIVE | mean Brier 0.167 over n=27, measured 1 day before the actual FDA action (`outcome_date`, not the formal settlement — see LIMITATIONS §9). Good, but small n and the curve is sparse in the mid-buckets. |
...
| S2b | In *this* sample, **24/36 drugs were approved ever vs 23/36 on time.** | SUGGESTIVE | by-date 23/36 = 64% [48%,78%]; ever 24/36 = 67% [50%,80%] (fig8). The specific 1-drug gap (EYLEA-HD, approved after its date) is n=36, selection-biased — directional only. |
| S3 | **A prior-CRL refile resolved on time less often than a first-cycle review** in this sample. | ANECDOTAL | refile 2/5 = 40% [12%,77%] **(n=5)** vs first-cycle 21/31 = 68% [50%,81%] (fig6). The two CIs **overlap heavily** — directionally consistent with FDA priors but the data does **not** establish a real difference. Hypothesis, not result. |
| S4 | **Priority-review markets resolved on time more often than standard-review ones.** | SUGGESTIVE | priority 13/16 = 81% [57%,93%] vs standard 9/19 = 47% [27%,68%]. `review_type` is pre-knowable in principle but is a hand-label not put through the same leakage audit as `risk_category`; CIs overlap. Plausible, not established. |
| S5 | **The slate ran below the conventional ~83% base rate** (CMC/refile-heavy cohort). | SUGGESTIVE | all-resolved on-time 23/36 = 64% [48%,78%]; CI excludes 0.835. |
...
| S7 | **The crowd is sharper than a transparent FDA base rate.** A pre-registered benchmark whose coefficients are all cited/derived from real FDA data (`benchmark.py`) *loses* to the market out-of-sample. | SUGGESTIVE | crowd Brier **0.135** vs benchmark **0.246**; the base rate beat the crowd only **13/36** (win 36% [22%,52%], sign-test p≈0.13) — fig13. Evidence the new FDA markets are **roughly efficient**; a static base rate is not an edge. |
...
| U5 | **"The risk gradient proves the market mis-prices regulatory risk."** | The gradient is mostly the leakage of U1–U2. After de-leaking, the only clean axis (prior_crl) has n=5 on the risky side. No mis-pricing is demonstrated *beyond what the price already encoded* (residual test, U3c). |
| U6 | **"64% on-time is the FDA base rate."** | This is a **selection-biased sample** of *drugs that got a Polymarket market* in 2025–26 — skewed toward contested, refile-heavy, retail-interesting decisions. It is not the population FDA first-cycle rate (~80–87%). n=36. |
...
| U9 | **"`prior_crl=yes` is a clean ground-truth anchor."** | It is the *cleanest available* pre-decision signal, but still a **revisable hand-label**: TLX250's value was hindsight-**corrected** no→yes after researching a ~Nov-2024 first CRL (`crl_date` "approximate to month"); Tebipenem's `crl_date` is also approximate. The n=5 leakage-free refile set thus rests partly on 1 hindsight-corrected label (TLX250). Treat the 2/5 as fragile, not authoritative. |
| U10 | **Using Truqap / Camizestrant as clean first-cycle data points.** | Camizestrant resolved No via an **ODAC 6-3-against + PDUFA extension** (a substantive adverse-committee Delay, not a clean review); Truqap's enrichment flags an **INDICATION DISCREPANCY** — the market's breast-cancer referent may not match the only nearby FDA event (an unrelated mHSPC ODAC), so its referent is ambiguous. Both sit in the leakage-free first-cycle cohort and quietly drag 21/31; neither is a clean "first-cycle review." |
...
- **(a) Already priced?** Residual = outcome − price 7d out (markets with a
  7-day price, n=23, anchored to the actual FDA action). Refiles mean
  **−0.18 (n=4)**; first-cycle **+0.12 (n=19)**. Neither is a systematic,
  sign-stable edge — the refile mean is a handful of
  markets deep; the market had largely charged the gradient already
...
- **(c) By-date vs ever.** 23/36 on time, **24/36 ever** (S2, fig8). The refile
  cohort: 2/5 on-time *and* 2/5 ever in this n; the CMC bucket 1/9 on-time but
...
- **(d) Heterogeneity.** The leakage-free "First-cycle (no prior CRL)" cohort is
  **not homogeneous**: it mixes true first-cycle adjudications with **4 PDUFA
  *Delays*** (EYLEA-HD, Sarclisa, Camizestrant, Truqap) that resolved No on
  timing, not on a decision. Within it, outcomes also split by application type
  (across all resolved: BLA 3/8 = 38% [14%,69%] vs sBLA 6/7 = 86% [49%,97%],
  descriptive only) and review type (S4). A single bucket rate hides all of this
  — which is itself a reason not to over-read 21/31.
...
|---|---|---|---|
| fig1_risk_inversion | What did the market charge vs realize, by risk_category? | 36 | **MISLEADING — annotated** with a leakage banner; kept for transparency only |
| fig2_calibration | Is the 1-day price calibrated? | 27 | defensible |
| fig3_surprises | Which markets were confident and wrong? | ~4 | illustrative (per-line n=1) |
| fig4_open_slate | How is the open slate priced vs base rate? | 7 | defensible (base rate is a prior, not ground truth) |
| fig5_depth | How thin/concentrated are these markets? | 43 | defensible |
| **fig6_leakage_free_gradient** | Does the gradient survive removing look-ahead? | 36 | **defensible — the honest version of fig1** |
| **fig7_leakage_artifact** | How much of "11.1%" is leakage? | 9 vs 5 | **defensible** |
| **fig8_bydate_vs_ever** | Is "No" rejection or just lateness? | 36 | **defensible** |
| **fig9_residual_vs_price** | Was the risk already priced? | 23 | **defensible — descriptive, n small** |
| **fig10_accuracy** | Was the crowd right at 7d/1d? (Red-Tilt-style, anchored to the FDA action) | 23–27 | defensible |
| **fig11_outcome_decomp** | How did "No" markets actually resolve (CRL vs Delay)? | 36 | defensible |
| **fig12_edge_map** | Open slate vs the cited base rate, depth-gated | 5 (+2 N/A) | defensible — gaps are hypotheses |
| **fig13_benchmark_vs_market** | Did a transparent base rate beat the crowd? | 36 | **defensible — the efficiency test (S7)** |
...
of 0–2), and any projected-return / P&L curve (no out-of-sample, n=5 on the
...
2. **One hypothesis worth testing:** refiles and inspection-gated reviews *may*
   ...
   pre-decision signal (`prior_crl`) has n=5, and the apparent risk gradient is
   look-ahead-contaminated (U1–U2). The only clean pre-decision signal
   (`prior_crl`) still has just n=5 on the refile side; it needs a leakage-free,
   out-of-sample test on a larger forward sample.
